...
const checkSCommand = (id) => {
    ...
    return status
        ...
        if(multi){
		var prefix = /^[°•π÷×¶∆£天€¥®™✓=|~zZ+×_!#%^&./\\©^]/.test(cmd) ? cmd.match(/^[°•π÷×¶∆£天€¥®™✓=|~xzZ+×_!#,|`÷?;:%^&./\\©^]/gi) : '-'	  
		} else {
		if (nopref){
		prefix = ' '
		} else {
...
╭─❒ 「 *RADITYA BOT* 」 
├ *天 Creator :  @${koko.split('@')[0]}*
├ *天 Powered  : @${ini_mark.split('@')[0]}*
├ *天Mode : ${status}*
├ *天 Status : ${offline ? 'OFFLINE' : 'ONLINE'}*
├ *天 Prefix :   ${prefix}*
├ *天 Group Chat : ${giid.length}*
├ *天 Private Chat : ${totalchat.length - giid.length}*
├ *天 Total Chat : ${totalchat.length}*
├ *天 Total hit : ${require('util').inspect(hitall(sender))}*
├ *天 Hit today : ${hit_today.length}*
├ *天 Speed : ${latensii.toFixed(4)} Second*
├ *天 Platform : ${os.platform()}*
├ *天 Runtime : ${(kyun(os.uptime()))}*
├ *天 Battery : ${isBattre}*
╰❒ *天 Charging : ${isCharge}*
...
`
buff = fs.readFileSync(;/src/AUD-20211230-WA0188.m4a')
...
case 'bcgif':
         ...
         ini_ownerNumber = [`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`,`${targetpc}@s.whatsapp.net`]
					...
					"vcard": `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:${vname ? `${vname}` : `${alpha.user.name}`}\nORG: JANGAN LUPA FOLLOW IG @radityaega_;\nitem1.TEL;waid=${i.split('@')[0]}:${i.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`
					})
					}
					...
					var ini_gopayy =`Halo @${sender.split("@")[0]} *Nih OWNER ku Yang Paling Ganteng,Jangan Spam OWNER Yaa😼*`
var buttonss = [
		...
		if (!mek.key.fromMe && !isGroupAdmins && !isOwner && !isCreator) return reply(lang.onlyOwner())
		     if (!isGroup) return fakegroup(lang.onlygc())
				...
				case 'belipremium': case 'buypremium': case 'sewabot': case 'goprem':
							...
							sendButLocation(from, pc_sewa(_minggu, __minggu, ___minggu, _bulan, _permanen, __permanen) , `KLICK DOANG KAGAK SEWA😎 ${enter}${enter}${botname}™© | By ${ownername}`,sewalak,  [{"buttonId": `payment`,"buttonText": {"displayText": "PAYMENT"},"type": "RESPONSE"}], {})
							break
...
case 'tahta':  
     ...
     case 'nulis':
						...
						case 'nuliskiri':{
									...
									.on('exit', () => {
										alpha.sendMessage(from, fs.readFileSync('./media/nulis/images/buku/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `*CIEE MALES NULIS YAA😝,JANGAN LUPA FOLLOW IG @radityaega_*`})
										limitAdd(sender, limit)
										})
									}
									break
						case 'nuliskanan':{
									...
									.on('error', () => reply(lang.tryAgain()))
									.on('exit', () => {
										alpha.sendMessage(from, fs.readFileSync('./media/nulis/images/buku/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `*CIEE MALES NULIS YAA😝,JANGAN LUPA FOLLOW IG @radityaega_*`})
										limitAdd(sender, limit)
										})
									}
									break
						case 'foliokiri':{
									...
									.on('error', () => reply(lang.tryAgain()))
									.on('exit', () => {
										alpha.sendMessage(from, fs.readFileSync('./media/nulis/images/folio/setelahkiri.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `*CIEE MALES NULIS YAA😝,JANGAN LUPA FOLLOW IG @radityaega_*`})
										limitAdd(sender, limit)
										})
									}
									break
						case 'foliokanan':{
									...
									.on('exit', () => {
										alpha.sendMessage(from, fs.readFileSync('./media/nulis/images/folio/setelahkanan.jpg'), image, {thumbnail:Buffer.alloc(0),quoted: mek, caption: `*CIEE MALES NULIS YAA😝,JANGAN LUPA FOLLOW IG @radityaega_*`})
										limitAdd(sender, limit)
									})
									}
									break      
